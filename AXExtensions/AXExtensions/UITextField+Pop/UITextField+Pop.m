...
- (void)pop_setAttributedText:(NSAttributedString *)attributedText {
    ...
    POPAnimatableProperty * nameProp = [POPAnimatableProperty propertyWithName:@"count" initializer:^(POPMutableAnimatableProperty *prop) {
        // read value
        prop.readBlock = ^(id obj, CGFloat values[]) {
            values[0] = MIN(++count, attributedText.length);
        };
        // write value
        prop.writeBlock = ^(id obj, const CGFloat values[]) {
            [obj setAttributedText:[attributedText attributedSubstringFromRange:NSMakeRange(0, values[0])]];
        };
        // dynamics threshold
        prop.threshold = 0.01;
    }];
    ...
    if (perSpeed > 0) {
        duration = perSpeed*attributedText.length;
    }
    POPBasicAnimation *nameAnim = [POPBasicAnimation animation];
    nameAnim.duration = duration;
    nameAnim.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseOut];
